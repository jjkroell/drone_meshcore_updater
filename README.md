...
> **This is a fork of [recrof/drone_meshcore_updater](https://github.com/recrof/drone_meshcore_updater)**
> that adds MeshCore status messages over LoRa on the RAK4631 — the updater
> narrates a DFU on a mesh channel, so it can be followed from the ground while
> it is out of Bluetooth range. See [MeshCore status over LoRa](#meshcore-status-over-lora-rak4631)
> below. Everything else is upstream's work.

A standalone updater that runs on a **Seeed XIAO** (nRF54LM20A, nRF52840,
ESP32-S3, ESP32-C5 or MG24) or a **RAK4631** with a RAK15001 flash module, holds
a library of firmware bundles, and flashes them into *other*
...
Everything below happens in **[the web client](https://jjkroell.github.io/drone_meshcore_updater/)**.
...
| `lora_freq` | `910.425` | MHz, decimals honoured |
