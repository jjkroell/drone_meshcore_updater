...
static void apply_defaults(struct app_config *c)
{
	 ...
	 *
	 * The radio defaults describe one specific mesh (910.425 MHz,
	 * 62.5 kHz, SF7, 4/5, +22 dBm) rather than a regional band plan,
	 * because a status message is only useful if it lands on the network
	 * the operator is standing in. Wrong-but-plausible defaults would
	 * transmit successfully and be heard by nobody, which is the worst of
	 * the available failures. Change them for a different mesh.
	 */
	snprintf(c->lora_channel, sizeof(c->lora_channel), "#drone-updater");
	snprintf(c->lora_sender, sizeof(c->lora_sender), "drone-updater");
	c->lora_freq_hz    = 910425000U;
